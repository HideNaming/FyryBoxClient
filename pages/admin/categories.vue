...
import Add from "~/components/admin/AddCategorie";
import Edit from "~/components/admin/EditCategorie";
  ...
  methods: {
    ...
    Delete(data) {
      this.$buefy.dialog.confirm({
        ...
        message:
          "Данная категория будет <b>удалёна</b> без возможности возврата.",
        confirmText: "Удалить",
        type: "is-danger",
        hasIcon: true,
        onConfirm: async () => {
          await this.$axios.delete("/api/v1/categories/" + data.id).then(() => {
            this.$buefy.toast.open("Категория удалена");
            this.Table();
          });
        },
      });
