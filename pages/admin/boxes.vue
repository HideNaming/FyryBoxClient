...
import Add from "~/components/admin/AddBox";
import Edit from "~/components/admin/EditBox";
  ...
  methods: {
    ...
    Delete(data) {
      this.$buefy.dialog.confirm({
        ...
        message:
          "Данная коробка будет <b>удалёна</b> без возможности возврата.",
        confirmText: "Удалить",
        type: "is-danger",
        hasIcon: true,
        onConfirm: async () => {
          await this.$axios.delete("/api/v1/boxes/" + data.id).then(() => {
            this.$buefy.toast.open("Коробка удалена");
            this.Table();
          });
        },
      });
