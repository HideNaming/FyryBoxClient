        ...
        <div class="content">
          ...
          <b-field v-if="!preLoading">
            <b-select
              ...
              expanded
            >
              <option v-for="item in boxes" :value="item.id" :key="item.id">
                {{ item.name }}
              </option>
            </b-select>
          </b-field>
          ...
          <b-field v-if="!preLoading" label="Информация">
...
export default {
  ...
  methods: {
    async Load() {
      await this.$axios.$get("/api/v1/boxes/").then((resp) => {
        this.boxes = resp;
        (async () => {
          await this.$axios
            .$get("/api/v1/loot/" + this.$parent.$parent.edit + "/edit")
            .then((response) => {
              this.form.name = response.name;
              this.form.price = response.price;
              this.form.box_id = response.box_id;
              this.form.stock = Boolean(response.stock);
    ...
    async Edit() {
      ...
      await this.$axios
        .$post("/api/v1/loot/" + this.$parent.$parent.edit, data, {
          headers: {
